...
You can promote the first admin in two ways:

1. **In-app (recommended)** — Sign in, open **Settings**. If no admin exists yet you'll see a banner **"Become the first administrator"**. Click it to claim the role. The button only works while no admin exists (server-checked).
2. **SQL** — From the Lovable Cloud SQL editor (uses `service_role`):

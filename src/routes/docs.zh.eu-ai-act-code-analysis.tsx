...
const DESC =
  "欧盟 AI 法案如何适用于 AI 代码审查与分析工具:风险等级、通用 AI 模型 (GPAI) 义务、透明度要求,以及欧洲工程团队的实务合规清单。";
...
const FAQ = [
  {
    q: "欧盟 AI 法案适用于 AI 代码审查工具吗?",
    a: "适用,只要在欧盟境内使用。大多数 AI 代码审查工具属于通用 AI (GPAI) 范畴,义务集中在透明度、技术文档、版权声明与告知下游用户。",
  },
  {
    q: "AI 代码分析属于高风险吗?",
    a: "默认不是。代码审查未列入附件三。只有当输出实质性决定受监管领域(关键基础设施、就业、信用评分、基本公共服务)的结果时,才升级为高风险。",
  },
  {
    q: "2025–2026 年 GPAI 提供方有何变化?",
    a: "必须发布模型卡、训练数据摘要与欧盟版权政策。下游部署方(您的团队)继承较轻但实在的义务:人在回路、透明度告知、技术文档。",
  },
  {
    q: "BYOK 如何帮助合规?",
    a: "BYOK 让模型提供方关系保留在您的 DPA 之下,工具变成瘦客户端,审计只需评估一个新的子处理方,而不是整条供应链。",
  },
];
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="mx-auto max-w-3xl px-4 py-12 sm:px-6 md:py-16">
        ...
        <p className="mt-4 text-lg text-muted-foreground">
          欧盟 AI 法案于 2024 年 8 月生效,2026–2027 年分阶段适用。如果您在欧洲交付软件并使用 AI
          读取、审查或生成代码,这里是面向工程团队的实务版本,以及为何 <strong>隐私优先、BYOK</strong>{" "}
          的工具(如 Decoder)契合新制度。
        </p>

        <Section icon={<Scale className="h-5 w-5" />} title="AI 代码审查在风险金字塔中的位置">
          <p>
            法案定义四级:<strong>不可接受</strong>、<strong>高风险</strong>、
            <strong>有限风险</strong>、<strong>最低风险</strong>。AI
            代码审查未列入附件三,默认处于有限风险层级。义务核心是<em>透明度</em>:告知 AI
            介入、记录所用模型、对上线决策保留人工复核。
          </p>
        </Section>
        <Section icon={<AlertTriangle className="h-5 w-5" />} title="GPAI:基础模型用户的变化">
          <p>
            2025 年 8 月起适用的 GPAI 章节将大部分义务放在<strong>提供方</strong>
            :模型卡、训练数据摘要、欧盟版权政策、对系统性风险模型的红队测试。
          </p>
          <p>
            作为<strong>下游部署方</strong>您承担较轻但具体的义务:告知审查者与开发者 AI
            在回路;记录提示词、留存与可访问性;对影响个人的决策保留人工复核;在基于内部代码微调时尊重版权保留。
          </p>
        </Section>
        <Section icon={<ShieldCheck className="h-5 w-5" />} title="为什么 BYOK 与本地推理简化合规">
          <p>
            BYOK 让模型提供方关系保留在您的 DPA
            之下。工具成为瘦客户端,审计只需评估一个新的子处理方。本地推理(Ollama、LM
            Studio)则彻底取消外部提供方,通常是金融、医疗、国防采购唯一能通过的路径。
          </p>
        </Section>

...
const FAQ = [
  {
    q: "What makes a code review 'secure' from a data perspective?",
    a: "Code that is reviewed should touch the minimum number of vendors. BYOK means code goes to one provider you already audited. Local models mean it never leaves your machine at all.",
  },
  {
    q: "Is BYOK code review compliant with SOC 2 / ISO 27001?",
    a: "Decoder's BYOK flow routes code to the provider you specify — the same one you likely already have a DPA with. Local model mode is zero-egress, which satisfies the strictest data residency requirements.",
  },
  {
    q: "How are API keys stored in Decoder?",
    a: "Keys are encrypted with AES-256-GCM at rest, stored in a row-level-security scoped table so only your user can read them, and never logged or sent to Decoder's own servers.",
  },
  {
    q: "Can I do a secure code review fully offline?",
    a: "Yes. Add an Ollama or LM Studio endpoint in Settings → Credentials. All inference — chunking, prompting, chat — runs locally. The static and malware pass are also offline by default.",
  },
  {
    q: "Which regulated industries use local-model code review?",
    a: "Finance, healthcare, and defence teams are the most common. Any team operating under data residency requirements, IP confidentiality clauses, or air-gap mandates benefits from local-only review.",
  },
];
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: DESC },
      {
        name: "keywords",
        content:
          "secure code review byok, secure ai code review, local model code review, air gap code review, private code review ai",
      },
      ...
      },
      {
        ...
        children: JSON.stringify({
          ...
          itemListElement: [
            ...
            { "@type": "ListItem", position: 3, name: "Secure code review with BYOK", item: URL },
          ],
        }),
      },
    ],
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="mx-auto max-w-3xl px-4 py-12 sm:px-6 md:py-16">
        ...
        <h1 className="mt-6 font-display text-4xl font-medium tracking-tight md:text-5xl">
          Secure code review with BYOK and local models
        </h1>
        <p className="mt-4 text-lg text-muted-foreground">
          Standard AI code review tools send your source code to a vendor's backend — adding a
          fourth party to your data chain without a DPA.{" "}
          <strong>Secure code review with BYOK</strong> means your code goes only to the provider
          you already chose and audited, or — with a local model — never leaves your machine at all.
        </p>

        <Section
          icon={<Lock className="h-5 w-5" />}
          title="The data-flow problem with managed review tools"
        >
          <p>When you use a managed AI code review SaaS, code travels:</p>
          <ol>
        ...
        <Section icon={<KeyRound className="h-5 w-5" />} title="BYOK: one provider, your account">
          <p>With Decoder's BYOK model, the flow collapses to:</p>
          <ol>
            <li>Your file → Decoder's analysis engine (static + malware, no external call).</li>
            <li>
              Flagged snippets → directly to your chosen provider (OpenAI, Anthropic, Google,
              OpenRouter) on your account.
            </li>
          </ol>
          <p>
            The review vendor (Decoder) never sees the content of the AI call. The model provider is
            the same one you likely already have a DPA or BAA with.
          </p>
          <ul>
        ...
        <Section icon={<Cpu className="h-5 w-5" />} title="Local models: zero egress">
          <p>
            ...
            classified code — Decoder also supports Ollama and LM Studio as local endpoints. In this
            mode:
          </p>
          <ul>
          ...
          <p>
            Recommended models for local secure review: <strong>Qwen2.5-Coder 14B</strong> (best
            quality), <strong>Llama 3.1 8B</strong> (fast, M-series Macs),{" "}
            <strong>DeepSeek-Coder-V2-Lite</strong> (lower VRAM).
          </p>
        </Section>
            ...
            <table className="w-full text-sm">
              <thead className="bg-muted/40 text-foreground">
                <tr>
                  ...
                  <th className="px-3 py-2 text-left font-medium">Vendors who see code</th>
                  <th className="px-3 py-2 text-left font-medium">Network egress</th>
                </tr>
              </thead>
              <tbody className="text-muted-foreground [&_td]:px-3 [&_td]:py-2 [&_tr]:border-t [&_tr]:border-border">
                <tr>
                  <td className="text-foreground">Managed SaaS (CodeRabbit)</td>
                  <td>Review vendor + model vendor</td>
                  <td>Yes</td>
                </tr>
                <tr>
                  <td className="text-foreground">Decoder + BYOK (cloud)</td>
                  <td>Model vendor only (your account)</td>
                  <td>Yes (your provider)</td>
                </tr>
                <tr>
                  <td className="text-foreground">Decoder + local Ollama</td>
                  <td>None</td>
                  <td>None</td>
                </tr>
              </tbody>
        ...
        <Section icon={<BookOpen className="h-5 w-5" />} title="Related guides">
          <ul>
            ...
            <li>
              <Link to="/docs/chat-with-your-codebase" className="text-foreground underline">
                Chat with your codebase using AI (BYOK)
              </Link>
            </li>
          </ul>
        </Section>

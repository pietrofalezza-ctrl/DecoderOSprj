...
const DESC = "为什么欧洲团队选择 BYOK、开源与本地推理。欧盟 AI 生态地图 (Mistral、Aleph Alpha、Silo AI)、采购标准与对比清单。";
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="mx-auto max-w-3xl px-4 py-12 sm:px-6 md:py-16">
        ...
        <div className="mt-6 flex gap-2 text-xs">
          ...
          <Link to="/docs/zh/privacy-first-ai-europe" className="rounded border border-border bg-card px-2 py-1 text-foreground">中文</Link>
        </div>
        <h1 className="mt-4 font-display text-4xl font-medium tracking-tight md:text-5xl">欧洲隐私优先 AI</h1>
        <p className="mt-4 text-lg text-muted-foreground">欧洲工程团队正在形成共识:<strong>BYOK</strong>、<strong>开源</strong>,在数据敏感时使用<strong>本地推理</strong>。本指南解释原因,列出 2026 年值得关注的欧洲玩家,并给出可直接放入供应商问卷的采购清单。</p>

        <Section icon={<ShieldCheck className="h-5 w-5" />} title="实务中的隐私优先">
          <ul>
            <li>模型提供方关系在<strong>您的</strong>合同下 (BYOK)。</li>
            <li>工具的提示词与规则<strong>可审计</strong>(开源)。</li>
            <li>必要时可端到端<strong>离线</strong>运行 (Ollama / LM Studio)。</li>
            <li>最小化存储,凭证<strong>静态加密</strong> (AES-256-GCM)。</li>
            <li>供应商不基于您的数据训练 — 写入合同,而非仅在博客。</li>
          </ul>
        </Section>
        <Section icon={<Flag className="h-5 w-5" />} title="值得关注的欧盟 AI 生态">
          <ul>
            <li><strong>Mistral</strong>(巴黎)— 开源权重与托管模型,欧盟端点,可经 OpenRouter 访问。</li>
            <li><strong>Aleph Alpha</strong>(海德堡)— 面向政府与受监管行业的主权部署。</li>
            <li><strong>Silo AI / AMD</strong>(赫尔辛基)— Poro 与 Viking 模型系列,聚焦欧洲语言。</li>
            <li><strong>OpenEuroLLM</strong> — 欧盟资助的多语种开源 LLM 联盟。</li>
            <li><strong>LightOn</strong>(法国)— 企业 RAG 与本地部署。</li>
            <li><strong>Hugging Face</strong> — 模型与数据集中心,欧盟托管 Inference Endpoints。</li>
          </ul>
        </Section>
        <Section icon={<GitCompare className="h-5 w-5" />} title="SaaS AI 对比隐私优先栈">
          <div className="not-prose overflow-x-auto rounded-lg border border-border">
            <table className="w-full text-sm">
              <thead className="bg-muted/40 text-foreground"><tr><th className="px-3 py-2 text-left font-medium">&nbsp;</th><th className="px-3 py-2 text-left font-medium">典型 SaaS AI</th><th className="px-3 py-2 text-left font-medium">Decoder (BYOK)</th><th className="px-3 py-2 text-left font-medium">Decoder(本地)</th></tr></thead>
              <tbody className="text-muted-foreground [&_td]:px-3 [&_td]:py-2 [&_tr]:border-t [&_tr]:border-border">
                <tr><td className="text-foreground">谁持有模型密钥</td><td>供应商</td><td>您</td><td>不适用</td></tr>
                <tr><td className="text-foreground">推理在何处运行</td><td>供应商云</td><td>您选择的提供方</td><td>您的机器</td></tr>
                <tr><td className="text-foreground">需审计的子处理方</td><td>多</td><td>一个 (Decoder)</td><td>零</td></tr>
                <tr><td className="text-foreground">基于您的数据训练</td><td>视套餐而定</td><td>否</td><td>否</td></tr>
                <tr><td className="text-foreground">源码可用</td><td>少</td><td>MIT</td><td>MIT</td></tr>
              </tbody>
            </table>
          </div>
        </Section>

...
const DESC =
  "源代码何时构成 GDPR 下的个人数据,为什么欧盟数据驻留对 AI 代码审查重要,以及 BYOK 加静态加密凭证如何降低控制者风险。";
...
const FAQ = [
  {
    q: "源代码会被视为个人数据吗?",
    a: "通常会。提交元数据中的作者姓名、邮箱、IP;注释中的姓名;包含真实标识符的测试数据;能识别个人操作者的密钥,都属于个人数据。",
  },
  {
    q: "Decoder 在哪里处理我的代码?",
    a: "ZIP 上传在 Cloudflare Workers 边缘运行时短暂处理后即删除。使用 BYOK 时模型调用直接发送到您账户下的提供方。使用本地推理 (Ollama / LM Studio) 时数据从不离开您的机器。",
  },
  {
    q: "我需要与 Decoder 签订 DPA 吗?",
    a: "如果您代表第三方处理个人数据,需要。我们是受托处理方,请联系 contact@decoderead.dev。使用本地推理时通常无需,因为数据留在您一侧。",
  },
  {
    q: "BYOK 密钥如何存储?",
    a: "采用 AES-256-GCM 静态加密。密文不暴露给浏览器,仅在服务端发起对外调用时解密。",
  },
];
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="mx-auto max-w-3xl px-4 py-12 sm:px-6 md:py-16">
        ...
        <p className="mt-4 text-lg text-muted-foreground">
          GDPR 未提及"代码",但当仓库包含作者标识、样本数据或操作者注释时,部分代码即成为
          <strong>个人数据</strong>。一旦 AI
          工具读取这些内容,您就有了需要治理的处理链。以下是欧洲工程团队的实务版本。
        </p>

        <Section icon={<FileText className="h-5 w-5" />} title="源代码何时构成个人数据">
          <ul>
            <li>
              <strong>提交元数据</strong> — 作者姓名、邮箱、IP。始终为个人数据。
            </li>
            <li>
              <strong>注释</strong> — 姓名、内部 Slack 账号、工单负责人。
            </li>
            <li>
              <strong>测试数据</strong> — 样本客户、真实邮箱、演示账号。
            </li>
            <li>
              <strong>日志与转储</strong> — 误提交的 IP、用户 ID,有时含凭证。
            </li>
            <li>
              <strong>密钥</strong> — 当密钥能识别个人操作者时同样适用。
            </li>
          </ul>
        </Section>
        <Section icon={<Globe className="h-5 w-5" />} title="数据驻留:字节实际落在哪里">
          <ul>
            <li>
              ZIP 上传与分析在 <strong>Cloudflare Workers</strong> 边缘运行时短暂处理后删除。
            </li>
            <li>
              <strong>BYOK</strong>:模型调用直接发往您账户下的提供方,适用您与该提供方的 DPA。
            </li>
            <li>
              <strong>本地推理</strong> (Ollama / LM Studio):所有内容留在本机。
            </li>
          </ul>
        </Section>
        <Section icon={<Lock className="h-5 w-5" />} title="凭证与存储">
          <p>
            BYOK 密钥使用 <strong>AES-256-GCM</strong>{" "}
            静态加密。密文从不返回浏览器,只暴露非敏感提示(末 4 位与提供方名称)用于
            UI。解密在服务端、对外调用时进行。详见{" "}
        ...
        <Section icon={<ShieldCheck className="h-5 w-5" />} title="DPA、子处理方与文档">
          <p>
            若您代表客户通过 Decoder 处理个人数据,Decoder 即为您的受托处理方,请通过{" "}
            <a href="mailto:contact@decoderead.dev" className="text-foreground underline">
              contact@decoderead.dev
            </a>{" "}
            申请 DPA。子处理方清单刻意保持简短:Cloudflare
            提供计算,事务邮件提供方负责通知。本地推理通常无子处理方。
          </p>
        </Section>
        ...
        <Section icon={<BookOpen className="h-5 w-5" />} title="相关指南">
          <ul>
            ...
            <li>
              <Link to="/docs/zh/privacy-first-ai-europe" className="text-foreground underline">
                欧洲隐私优先 AI
              </Link>
            </li>
            <li>

...
const FAQ = [
  {
    q: "Can I chat with my codebase without sending code to OpenAI?",
    a: "Yes. Decoder supports Ollama and LM Studio as local endpoints. Chunking, prompting, and chat all run on your machine — no bytes leave the network.",
  },
  {
    q: "What BYOK providers does codebase chat support?",
    a: "OpenAI, Anthropic, Google Gemini, and OpenRouter for cloud models. Keys are encrypted AES-256-GCM at rest and scoped to your account only.",
  },
  {
    q: "How much context can Decoder send in a codebase chat?",
    a: "Decoder chunks files and sends relevant chunks per turn. For large codebases, upload a ZIP and the relevant files are retrieved per query.",
  },
  {
    q: "Is codebase chat useful without an API key?",
    a: "The static and malware scan always run. Chat requires a model — either a BYOK key or a local Ollama/LM Studio endpoint.",
  },
  {
    q: "Which local model works best for codebase chat?",
    a: "Qwen2.5-Coder 14B or 32B gives the best results on consumer hardware. Llama 3.1 8B is a good starting point for machines with less VRAM.",
  },
];
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: DESC },
      {
        name: "keywords",
        content:
          "chat with your codebase, ai codebase chat, byok codebase chat, local llm codebase, ollama codebase chat",
      },
      ...
      },
      {
        ...
        children: JSON.stringify({
          ...
          itemListElement: [
            ...
            { "@type": "ListItem", position: 3, name: "Chat with your codebase", item: URL },
          ],
        }),
      },
    ],
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="mx-auto max-w-3xl px-4 py-12 sm:px-6 md:py-16">
        ...
        <Section icon={<MessageSquare className="h-5 w-5" />} title="What codebase chat unlocks">
          <ul>
            <li>
              <strong>Instant orientation</strong> — drop into an unfamiliar repo and ask the model
              to explain the architecture before reading a single file.
            </li>
            <li>
              <strong>Targeted security questions</strong> — "does this module validate user input
              before the DB call?" is faster than tracing the call graph manually.
            </li>
            <li>
              <strong>Refactor planning</strong> — ask the model what else would break if you
              changed a shared interface, getting a checklist before you write a line.
            </li>
            <li>
              <strong>Finding logic duplication</strong> — "does this utility already exist
              somewhere in the codebase?" saves hours on large repos.
            </li>
          </ul>
        </Section>
        ...
        <Section icon={<KeyRound className="h-5 w-5" />} title="BYOK: your key, your provider">
          ...
          <p>
            Typical cost for a session of 20 chat turns on a 1,000-line file: under $0.05 on
        ...
        <Section
          ...
          title="Fully local chat with Ollama or LM Studio"
        >
          <p>
            For regulated code or IP-sensitive work, add an Ollama or LM Studio endpoint in Settings
            → Credentials. From that point every chat turn — chunking, embedding, generation — runs
            on your hardware. Zero bytes leave the machine.
          </p>
          <ul>
            <li>
              <strong>Qwen2.5-Coder 14B / 32B</strong> — best quality-per-VRAM ratio for
              code-specific chat.
            </li>
            <li>
              <strong>Llama 3.1 8B Instruct</strong> — fast, fits on M-series Macs (16 GB unified
              memory).
            </li>
            <li>
              <strong>DeepSeek-Coder-V2-Lite</strong> — strong reasoning at a smaller footprint.
            </li>
          </ul>
        </Section>
        ...
        <Section
          ...
          title="BYOK chat vs managed codebase tools"
        >
            ...
            <table className="w-full text-sm">
              <thead className="bg-muted/40 text-foreground">
                <tr>
                  ...
                  <th className="px-3 py-2 text-left font-medium">Cursor / GitHub Copilot Chat</th>
                </tr>
              </thead>
              <tbody className="text-muted-foreground [&_td]:px-3 [&_td]:py-2 [&_tr]:border-t [&_tr]:border-border">
                <tr>
                  ...
                  <td>Any (BYOK or local)</td>
                  <td>Vendor-selected</td>
                </tr>
                <tr>
                  <td className="text-foreground">Code leaves machine</td>
                  <td>Only if you choose cloud key</td>
                  <td>Always</td>
                </tr>
                <tr>
                  <td className="text-foreground">Offline / air-gapped</td>
                  <td>Yes (local model)</td>
                  <td>No</td>
                </tr>
                <tr>
                  <td className="text-foreground">Static + malware pre-check</td>
                  <td>Built-in</td>
                  <td>No</td>
                </tr>
                <tr>
                  <td className="text-foreground">Pricing</td>
                  <td>Metered BYOK spend</td>
        ...
        <Section icon={<ShieldCheck className="h-5 w-5" />} title="Security of codebase chat">
          <p>
            ...
            deterministic scanner surfaces IOC matches first, so you can decide whether to involve a
            cloud model at all.
          </p>
        </Section>

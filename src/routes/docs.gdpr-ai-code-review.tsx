  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: DESC },
      {
        name: "keywords",
        content:
          "gdpr ai code review, ai code analysis gdpr, data residency ai code, source code personal data, ai dpa europe",
      },
    ...
    links: [
      ...
      { rel: "alternate", hrefLang: "en", href: URL },
      {
        rel: "alternate",
        hrefLang: "it",
        href: "https://decoderead.dev/docs/it/gdpr-revisione-codice-ai",
      },
      {
        rel: "alternate",
        hrefLang: "zh",
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="mx-auto max-w-3xl px-4 py-12 sm:px-6 md:py-16">
        ...
        <p className="mt-4 text-lg text-muted-foreground">
          GDPR doesn't mention "code", but the moment a repository contains author identifiers,
          sample data, or operator comments, parts of it become
          <strong> personal data</strong>. Once an AI tool reads that code, you have a processing
          chain to govern. Here's the short, practical version for engineering teams in Europe.
        </p>

        <Section
          icon={<FileText className="h-5 w-5" />}
          title="When source code becomes personal data"
        >
          <ul>
            <li>
              <strong>Commit metadata</strong> — author name, email, IP. Always personal data.
            </li>
            <li>
              <strong>Comments</strong> — names, ticket assignees, internal slack handles.
            </li>
            <li>
              <strong>Test fixtures</strong> — sample customers, real-looking emails, demo accounts.
            </li>
            <li>
              <strong>Logs and dumps</strong> committed by mistake — IPs, user IDs, sometimes
              credentials.
            </li>
            <li>
              <strong>Secrets</strong> — when a key identifies an individual operator, it counts
              too.
            </li>
          </ul>
          <p>
            If any of these end up in a prompt to a third-party LLM, that LLM provider becomes a
            sub-processor and you need a legal basis plus the right contractual cover.
          </p>
        </Section>

        <Section
          icon={<Globe className="h-5 w-5" />}
          title="Data residency: where the bytes actually land"
        >
          <p>
            EU teams usually need to know two things:{" "}
            <em>where does the review tool process my code</em>, and{" "}
            <em>where does the model run</em>. Decoder's answer:
          </p>
          <ul>
            <li>
              ZIP uploads and analysis run in <strong>Cloudflare Workers</strong> — globally
              distributed but encrypted in transit, processed ephemerally, and deleted after the
              analysis.
            </li>
            <li>
              <strong>BYOK</strong> means the model call goes directly to the provider on{" "}
              <em>your</em> account, under <em>your</em> DPA with that provider. You pick OpenAI,
              Anthropic, Google or OpenRouter, and you pick the region they offer.
            </li>
            <li>
              <strong>Local inference</strong> (Ollama / LM Studio) keeps everything on your machine
              — the cleanest residency story possible.
            </li>
          </ul>
        </Section>
        ...
        <Section icon={<Lock className="h-5 w-5" />} title="Credentials and storage">
          <p>
            BYOK keys are encrypted at rest with <strong>AES-256-GCM</strong> using a server-side
            key. The encrypted ciphertext is never returned to the browser — only a non-sensitive
            hint (last 4 characters and provider name) is exposed for UI display. Decryption happens
            server-side, at the exact moment of an outbound provider call, and the plaintext lives
            only in memory for that request.
          </p>
          <p>
        ...
        <Section
          ...
          title="DPA, sub-processors and your paperwork"
        >
          <p>
            If you process personal data through Decoder on behalf of customers (typical for
            agencies, consultancies, SaaS vendors), Decoder is your processor and you need a DPA.
            Request one at{" "}
            <a href="mailto:contact@decoderead.dev" className="text-foreground underline">
              contact@decoderead.dev
            </a>
            . Sub-processors are listed in the privacy page and kept short on purpose: Cloudflare
            for compute and a transactional email provider for notifications. With local inference
            there are usually no sub-processors at all.
          </p>
        </Section>
        ...
        <Section icon={<BookOpen className="h-5 w-5" />} title="Related guides">
          <ul>
            <li>
              <Link to="/docs/eu-ai-act-code-analysis" className="text-foreground underline">
